  ...
  <div class="main">
    ...
    <div class="divRoomTop2">
      <ul>
        ...
        <li v-bind:class="{liRoomTopC:golbal.typeCur=='scene'}" @click="typeClick('scene')"><i class="icon-ls_26_go_off_work"></i></li>
      </ul>
    </div>
    <div class="divRoom" :style="'height:'+heightRoom+'px'">
      ...
      <div :class="'divRoomContent'+ (isGallery?' divRoomGallery':' divRoomList')">
        <template v-for="device in golbal.roomCur.devices">
          ...
          <DeviceDimmer v-if="(device.type=='dimmer' && (device.type == golbal.typeCur || golbal.typeCur=='all' || golbal.typeCur=='light'))&&device.isFavorite" v-bind:key="device.id" v-bind:device="device" ></DeviceDimmer>
