    ...
    <div class="divRoom" :style="'height:'+heightRoom+'px'">
      ...
      <div :class="'divRoomContent'+ (isGallery?' divRoomGallery':' divRoomList')">
        <template v-for="device in golbal.roomCur.devices">
          ...
          <DeviceDimmer v-if="(device.type=='dimmer' && (device.type == golbal.typeCur || golbal.typeCur=='all'))&&device.isFavorite" v-bind:key="device.id" v-bind:device="device" ></DeviceDimmer>
