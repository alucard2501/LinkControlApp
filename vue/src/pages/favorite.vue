  ...
  <div class="main">
    ...
    <div class="divRoom" :style="'height:'+heightRoom+'px'">
      <template v-for="device in golbal.roomCur.devices">
        <DeviceLight v-if="(device.type=='light')&&device.isFavorite" v-bind:key="device.id" v-bind:device="device" ></DeviceLight>
        <DeviceCurtain v-if="(device.type=='curtain')&&device.isFavorite" v-bind:key="device.id" v-bind:device="device" ></DeviceCurtain>
        <DeviceDimmer v-if="(device.type=='dimmer')&&device.isFavorite" v-bind:key="device.id" v-bind:device="device" ></DeviceDimmer>
        <DeviceAC v-if="(device.type=='ac')&&device.isFavorite" v-bind:key="device.id" v-bind:device="device"></DeviceAC>
      </template>
    </div>
  </div>
</template>
