...
@interface ChromeSocketsUdpSocket ()<GCDAsyncUdpSocketDelegate>
...
-(void) initUdp{
    ...
    //[_udpSocket bindToPort:6668 error:&error];
    //[_udpSocket connectToHost:@"192.168.0. onPort:<#(uint16_t)#> error:<#(NSError *__autoreleasing *)#>]
...
- (void)sendBroadcast{
    char arr[] ={0xff, 0x01, 0x01, 0x02};
    NSData * data = [NSData dataWithBytes:arr length:4];
    [_udpSocket sendData:data toHost:@"255.255.255.255" port:1901 withTimeout:2 tag:0];
...
- (void)send:(CDVInvokedUrlCommand*)command
{
    ...
    [socket->_socket sendData:data toHost:address port:port withTimeout:2 tag:0];
